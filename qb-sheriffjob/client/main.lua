...
RegisterNetEvent('sheriff:client:sendBillingMail')
AddEventHandler('sheriff:client:sendBillingMail', function(amount)
    SetTimeout(math.random(2500, 4000), function()
        ...
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            ...
            message = "Dear " .. gender .. " " .. charinfo.lastname .. ",<br /><br />The Central Judicial Collection Agency (CJCA) charged the fines you received from the sheriff.<br />There is <strong>$"..amount.."</strong> withdrawn from your account.<br /><br />Kind regards,<br />Mr. I.K. Graai",
            button = {}
        })
    end)
end)
...
RegisterNetEvent('sheriff:client:toggleDatabank')
AddEventHandler('sheriff:client:toggleDatabank', function()
...
local DutyBlips = {}
RegisterNetEvent('sheriff:client:UpdateBlips')
AddEventHandler('sheriff:client:UpdateBlips', function(players)

    if PlayerJob ~= nil and (PlayerJob.name == 'sheriff' or PlayerJob.name == 'ambulance' or PlayerJob.name == 'doctor') and onDuty then
...
RegisterNetEvent('sheriff:client:SendsheriffEmergencyAlert')
AddEventHandler('sheriff:client:SendsheriffEmergencyAlert', function()
    ...
    TriggerServerEvent("sheriff:server:SendsheriffEmergencyAlert", streetLabel, pos, QBCore.Functions.GetPlayerData().metadata["callsign"])
    TriggerServerEvent('qb-sheriffalerts:server:AddsheriffAlert', {
        timeOut = 10000,
...
RegisterNetEvent('sheriff:PlaySound')
AddEventHandler('sheriff:PlaySound', function()
    PlaySound(-1, "Lose_1st", "GTAO_FM_Events_Soundset", 0, 0, 1)
end)

RegisterNetEvent('sheriff:client:sheriffEmergencyAlert')
AddEventHandler('sheriff:client:sheriffEmergencyAlert', function(callsign, streetLabel, coords)
    if (PlayerJob.name == 'sheriff' or PlayerJob.name == 'ambulance' or PlayerJob.name == 'doctor') and onDuty then
        local transG = 250
...
RegisterNetEvent('sheriff:client:GunShotAlert')
AddEventHandler('sheriff:client:GunShotAlert', function(streetLabel, isAutomatic, fromVehicle, coords, vehicleInfo)
    if PlayerJob.name == 'sheriff' and onDuty then        
        ...
        TriggerEvent('qb-sheriffalerts:client:AddsheriffAlert', {
            timeOut = 4000,
            alertTitle = blipText,
            coords = {
                x = coords.x,
...
RegisterNetEvent('sheriff:client:VehicleCall')
AddEventHandler('sheriff:client:VehicleCall', function(pos, alertTitle, streetLabel, modelPlate, modelName)
    if PlayerJob.name == 'sheriff' and onDuty then
        TriggerEvent('qb-sheriffalerts:client:AddsheriffAlert', {
            timeOut = 4000,
            alertTitle = alertTitle,
...
RegisterNetEvent('sheriff:client:HouseRobberyCall')
AddEventHandler('sheriff:client:HouseRobberyCall', function(coords, msg, gender, streetLabel)
    if PlayerJob.name == 'sheriff' and onDuty then
        TriggerEvent('qb-sheriffalerts:client:AddsheriffAlert', {
            timeOut = 5000,
...
RegisterNetEvent('112:client:SendsheriffAlert')
AddEventHandler('112:client:SendsheriffAlert', function(notifyType, data, blipSettings)
    if PlayerJob.name == 'sheriff' and onDuty then
...
RegisterNetEvent('sheriff:client:sheriffAlertMessage')
AddEventHandler('sheriff:client:sheriffAlertMessage', function(title, streetLabel, coords)
    if PlayerJob.name == 'sheriff' and onDuty then
        TriggerEvent('qb-sheriffalerts:client:AddsheriffAlert', {
            timeOut = 5000,
            alertTitle = title,
...
RegisterNetEvent('sheriff:server:SendEmergencyMessageCheck')
AddEventHandler('sheriff:server:SendEmergencyMessageCheck', function(MainPlayer, message, coords)
    local PlayerData = QBCore.Functions.GetPlayerData()
    if ((PlayerData.job.name == "sheriff" or PlayerData.job.name == "ambulance" or PlayerData.job.name == "doctor") and onDuty) then
        ...
        TriggerEvent("sheriff:client:EmergencySound")
        local transG = 250
        local blip = AddBlipForCoord(coords.x, coords.y, coords.z)
...
RegisterNetEvent('sheriff:client:Send112AMessage')
AddEventHandler('sheriff:client:Send112AMessage', function(message)
    local PlayerData = QBCore.Functions.GetPlayerData()
    if ((PlayerData.job.name == "sheriff" or PlayerData.job.name == "ambulance") and onDuty) then
        TriggerEvent('chatMessage', "ANONYMOUS REPORT", "warning", message)
        TriggerEvent("sheriff:client:EmergencySound")
    end
end)

RegisterNetEvent('sheriff:client:SendToJail')
AddEventHandler('sheriff:client:SendToJail', function(time)
    TriggerServerEvent("sheriff:server:SetHandcuffStatus", false)
    isHandcuffed = false
    isEscorted = false
    ClearPedTasks(PlayerPedId())
