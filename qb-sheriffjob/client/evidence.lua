			...
			if PlayerJob.name ~= "sheriff" then
				if IsPedShooting(PlayerPedId()) and not IsSilentWeapon(currentWeapon) and IsPedNearby() then
					...
					if IsPedInAnyVehicle(PlayerPedId(), true) then
						...
						TriggerServerEvent("sheriff:server:GunshotAlert", streetLabel, automatic, true, coords, vehicleInfo)
					else
						TriggerServerEvent("sheriff:server:GunshotAlert", streetLabel, automatic, false, coords)
					end
					Citizen.Wait(15000)
				end
...
function IsSilentWeapon(weapon)
	...
	if not retval then 
		QBCore.Functions.TriggerCallback('sheriff:IsSilencedWeapon', function(result)
			retval = result
			return result
		end, weapon)
		Citizen.Wait(100)
